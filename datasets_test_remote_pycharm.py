...
def default_flist_reader_fungi(flist):
    """
    flist format: impath label\nimpath label\n ...(same to caffe's filelist)
    """
    imlist = []
...
class ImageFilelist_fungi(data.Dataset):
    def __init__(self, flist, transform=None, target_transform=None,
                 flist_reader=default_flist_reader_fungi, loader=default_loader):
        self.imlist = flist_reader(flist)
        self.transform = transform
        self.target_transform = target_transform
        self.loader = loader
...
def fire_prediction_DB(batch_size, train_list, val_list, train=True, val=True, **kwargs):
    ...
    val_list = './data/image_burn_pred_val_20190701to20190801.txt'
...
# def fire_prediction_DB(batch_size, train_list, val_list, train=True, val=True, **kwargs):
#
#     train_list = './data/image_burn_pred_20190701to20190801.txt'
#     val_list = './data/image_burn_pred_20190701to20190801.txt'
#     num_workers = kwargs.setdefault('num_workers', 1)
#     kwargs.pop('input_size', None)
#     print("Building data loader with {} workers".format(num_workers))
#     ds = []
...
#                     # transforms.RandomHorizontalFlip(),
#                     transforms.ToTensor(),
#                     transforms.Normalize((0.00023, 0.074, 0.20, 0.17, 0.12, 0.77, 0.50, 0.41, 0.11, 0.58, 0.47, 0.15, 0.31), (0.015, 0.24, 0.26, 0.22, 0.14, 0.15, 0.23, 0.23, 0.14, 0.25, 0.28, 0.20, 0.24)),
#                 ])),
#             batch_size=batch_size, shuffle=True, **kwargs)
#         print("Training data size: {}".format(len(train_loader.dataset)))
#         ds.append(train_loader)
